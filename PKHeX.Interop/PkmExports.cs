...
{
    /// <summary>
    /// Resource cache over this assembly. PKHeX.Core exposes
    /// <see cref="EmbeddedResourceCache"/> / <see cref="Util.GetStringList(string, EmbeddedResourceCache)"/>
    /// so downstream consumers can carry their own string tables; we use it for supplemental data
    /// PKHeX.Core ships none of, instead of patching the submodule.
    /// </summary>
    private static readonly EmbeddedResourceCache InteropResources = new(typeof(PkmExports).Assembly);

    /// <summary>
    /// Ability flavor-text, indexed by ability ID to match <see cref="GameStrings.abilitylist"/>.
    /// Generated by Scripts/generate_ability_descriptions.py into Resources/; IDs with no catalogued
    /// description are empty lines. Only English exists, so the language is not parameterized.
    /// </summary>
    private static string[] AbilityDescriptions => Util.GetStringList("text_AbilityDescriptions_en", InteropResources);
